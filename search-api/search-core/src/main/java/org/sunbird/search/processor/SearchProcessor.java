...
public class SearchProcessor {
    ...
    public Future<Map<String, Object>> processSearch(SearchDTO searchDTO, boolean includeResults, boolean  fuzzyEnabled)
            throws Exception {
        List<Map<String, Object>> groupByFinalList = new ArrayList<Map<String, Object>>();
        SearchSourceBuilder query = processSearchQuery(searchDTO, groupByFinalList, true, fuzzyEnabled);
        Future<SearchResponse> searchResponse = ElasticSearchUtil.search(
                SearchConstants.COMPOSITE_SEARCH_INDEX,
                query);

        int exactMatchCount = ElasticSearchUtil.count(SearchConstants.COMPOSITE_SEARCH_INDEX,
                query);

        if(exactMatchCount == 0){
            query = processSearchQuery(searchDTO, groupByFinalList, true, true);
    ...
    private SearchSourceBuilder processSearchQuery(SearchDTO searchDTO, List<Map<String, Object>> groupByFinalList,
                                                   boolean sortBy, boolean fuzzyEnabled) {
        ...
        if (searchDTO.getFacets() != null && groupByFinalList != null && !fuzzyEnabled) {
            for (String facet : searchDTO.getFacets()) {
                Map<String, Object> groupByMap = new HashMap<String, Object>();
                groupByMap.put("groupByParent", facet);
                groupByFinalList.add(groupByMap);
        ...
        searchSourceBuilder.size(searchDTO.getLimit());
        searchSourceBuilder.from(searchDTO.getOffset());
        QueryBuilder query = getSearchQuery(searchDTO, fuzzyEnabled);
        if (searchDTO.isFuzzySearch())
            relevanceSort = true;
    ...
    private QueryBuilder prepareSearchQuery(SearchDTO searchDTO, boolean fuzzyEnabled) {
        BoolQueryBuilder boolQuery = new BoolQueryBuilder();
        QueryBuilder queryBuilder = null;
        String totalOperation = searchDTO.getOperation();
        List<Map> properties = searchDTO.getProperties();
        formQuery(properties, queryBuilder, boolQuery, totalOperation, fuzzyEnabled);
        if (searchDTO.getMultiFilterProperties() != null) {
            formQuery(searchDTO.getMultiFilterProperties(), queryBuilder, boolQuery,
                    SearchConstants.SEARCH_OPERATION_OR, fuzzyEnabled);
        }
    ...
    private QueryBuilder getSearchQuery(SearchDTO searchDTO, boolean fuzzyEnabled) {
        BoolQueryBuilder boolQuery = new BoolQueryBuilder();
        QueryBuilder origFilterQry = getQuery(searchDTO, fuzzyEnabled);
        QueryBuilder implFilterQuery = null;
    ...
    private QueryBuilder getQuery(SearchDTO searchDTO, boolean fuzzyEnabled) {
        return prepareSearchQuery(searchDTO, fuzzyEnabled);
    }
